...
from .forms import LoanRequestForm, LoanApplicationDetailsForm, LoanTransactionForm
from .models import loanRequest, loanTransaction, CustomerLoan
from django.shortcuts import redirect
from django.http import HttpResponseRedirect, HttpResponse
from django.shortcuts import get_object_or_404, redirect, render
from .forms import LoanRequestForm, LoanApplicationDetailsForm
...
def LoanRequest(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            loan_obj.save()
            # Redirect to application details page after saving
            return redirect('loanApp:application_details', loan_id=loan_obj.id)
        else:
            print(form.errors)  # DEBUG: check why form is invalid
    else:
...
@login_required(login_url='/account/login-customer')
def application_details_view(request, loan_id):
    loan = get_object_or_404(loanRequest, id=loan_id)

    if request.method == 'POST':
        form = LoanApplicationDetailsForm(request.POST, request.FILES, instance=loan)
        if form.is_valid():
            form.save()
            messages.success(request, 'Application details updated successfully!')
            return redirect('loanApp:user_dashboard')
    else:
        form = LoanApplicationDetailsForm(instance=loan)
    ...
    return render(request, 'loanApp/application_details.html', context)
...
def application_details_view(request, loan_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('loanApp:user_dashboard')  # redirect after saving
    else:
        form = LoanApplicationDetailsForm(instance=loan)
...
def terms_conditions(request):
    return render(request, 'loanApp/terms.html')
